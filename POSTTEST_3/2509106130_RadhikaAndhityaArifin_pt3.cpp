...
struct Stack {
    Pemesan data[MAX_STK];
    int top;
    Stack() : top(-1) {}
    bool isFull()  { return top >= MAX_STK - 1; }
...
Pemesan dequeue(Queue& q, bool& berhasil) {
    ...
    Pemesan p = q.data[q.front];
    q.front++;
    if (q.isEmpty()) q.front = q.rear = -1;
    berhasil = true;
    return p;
}
...
void peekQueue(const Queue& q) {
    ...
    const Pemesan* p = &q.data[q.front];
...
void tampilQueue(const Queue& q) {
    ...
    cout << "  Total antrian: " << (q.rear - q.front + 1) << " penumpang.\n";
}
...
void tampilStack(const Stack& s) {
    ...
    for (int i = s.top; i >= 0; i--) {
        const Pemesan* p = s.data + i;
        cout << "  [" << (s.top - i + 1) << "] Nama        : " << p->namaPenumpang << "\n";
        cout << "      Kereta      : " << p->namaKereta    << "\n";
        cout << "      Rute        : " << p->asal << " -> " << p->tujuan << "\n";
        cout << "      Harga Tiket : Rp " << p->hargaTiket << "\n";
        cout << "  ------------------------------------------------------------\n";
